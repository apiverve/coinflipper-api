...
class CoinflipperValidationException implements Exception {
  final List<String> errors;

  CoinflipperValidationException(this.errors);

  @override
  String toString() => 'CoinflipperValidationException: ${errors.join("; ")}';
}
...
/// Coin Flipper API client.
///
/// For more information, visit: https://apiverve.com/marketplace/coinflipper?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [flips] - Number of coins to flip [min: 1, max: 10000]
class CoinflipperClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'flips': ValidationRule(type: 'integer', required: false, min: 1, max: 10000),
  };

  CoinflipperClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/coinflipper',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CoinflipperValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CoinflipperValidationException(errors);
    }
  }

  /// Execute a request to the Coin Flipper API.
  ///
  /// Parameters are validated before sending the request.
  Future<CoinflipperResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CoinflipperException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CoinflipperResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CoinflipperException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CoinflipperException('Resource not found');
      } else {
        throw CoinflipperException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CoinflipperException) rethrow;
      throw CoinflipperException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Coin Flipper API client.
class CoinflipperException implements Exception {
  final String message;

  CoinflipperException(this.message);

  @override
  String toString() => 'CoinflipperException: $message';
}
