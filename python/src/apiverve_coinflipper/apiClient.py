...
class CoinflipperAPIClientError(Exception):
...
class ValidationError(CoinflipperAPIClientError):
...
class CoinflipperAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"flips": {"type": "integer", "required": False, "min": 1, "max": 10000, "default": 1}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the CoinflipperAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises CoinflipperAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/coinflipper'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise CoinflipperAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[CoinflipperAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns CoinFlipperData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises CoinflipperAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise CoinflipperAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise CoinflipperAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise CoinflipperAPIClientError(f"Request failed: {str(e)}")
